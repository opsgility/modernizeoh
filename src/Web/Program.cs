...
if (builder.Environment.IsDevelopment() || builder.Environment.EnvironmentName == "Docker"){
    ...
    if (useKeyVault == true)
    {
        // Read Vault URI from config
        var keyVaultUri = builder.Configuration["KeyVault:VaultUri"];

        // Setup DefaultAzureCredential (uses Managed Identity automatically in Azure)
        var credential = new DefaultAzureCredential();
        var client = new SecretClient(new Uri(keyVaultUri), credential);

        // Read secret (sync or async)
        KeyVaultSecret secret = client.GetSecret("CatalogConnection"); // Use async in production
        string catalogConnectionString = secret.Value;

        secret = client.GetSecret("IdentityConnection"); // Use async in production
        string IdentityConnectionString = secret.Value;
